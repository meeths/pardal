...
lvk::Result lvk::compileShaderGlslang(lvk::ShaderStage stage,
  ...
  if (!glslang_shader_preprocess(shader, &input)) {
    pdlLogWarning("Shader preprocessing failed:\n");
  ...
  if (!glslang_shader_parse(shader, &input)) {
    pdlLogWarning("Shader parsing failed:\n");
    pdlLogWarning("  %s\n", glslang_shader_get_info_log(shader));
    pdlLogWarning("  %s\n", glslang_shader_get_info_debug_log(shader));
  ...
  if (!glslang_program_link(program, GLSLANG_MSG_SPV_RULES_BIT | GLSLANG_MSG_VULKAN_RULES_BIT)) {
    pdlLogWarning("Shader linking failed:\n");
...
lvk::Result lvk::compileShaderSlang(lvk::ShaderStage stage,
  ...
  slang::IModule* slangModule = nullptr;
  {
    ...
    if (diagnosticBlob) {
      pdlLogWarning("%s", (const char*)diagnosticBlob->getBufferPointer());
    }
    if (!slangModule) {
